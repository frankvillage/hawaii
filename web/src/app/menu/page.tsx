...
import { menuHighlights, menuSections } from "@/lib/site-content";
        ...
        <div className="mt-14 grid gap-5">
          {menuSections.map((section) => (
            <article
              ...
              className="grid gap-5 rounded-[2rem] border border-white/10 bg-[rgba(255,255,255,0.04)] p-6 lg:grid-cols-[0.52fr_0.48fr]"
            >
              <div>
                <p className="text-[0.68rem] uppercase tracking-[0.22em] text-[#d6b887]">
                  {section.eyebrow}
                </p>
                <h2 className="mt-4 max-w-md font-serif text-3xl text-[#f4ede4] sm:text-4xl">
                  {section.title}
                </h2>
                <p className="mt-4 max-w-xl text-base leading-8 text-[#c5d1d8]">
                  {section.description}
                </p>
                {section.action ? (
                  <Link
              ...
              <ul className="grid gap-3 text-sm leading-7 text-[#d9e2e7]">
                {section.items.map((item) => (
                  <li
                    key={item}
                    className="rounded-[1.25rem] border border-white/10 bg-[rgba(7,17,26,0.32)] px-4 py-3"
                  >
                    {item}
                  </li>
                ))}
