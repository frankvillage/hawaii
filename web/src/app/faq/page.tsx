  ...
  return (
    <main className="theme-light bg-[#f8f5ee]">
      ...
      <section className="mx-auto max-w-4xl px-4 pt-32 pb-16 sm:px-6 lg:px-8">
        <p className="text-[0.72rem] uppercase tracking-[0.24em] text-[#96703d]">FAQ</p>
        <h1 className="mt-5 font-serif text-5xl leading-[0.92] text-[#16292d] sm:text-6xl">
          ...
          {faqGroups.map((group) => (
            <section key={group.title}>
              <div className="max-w-2xl">
                ...
                <p className="mt-3 text-base leading-8 text-[#4c5453]">{group.intro}</p>
              </div>
              <div className="mt-5 space-y-5">
                {group.items.map((faq) => (
                  ...
                  >
                    <h2 className="text-xl font-semibold text-[#16292d]">{faq.question}</h2>
                    <p className="mt-3 text-sm leading-7 text-[#4c5453]">{faq.answer}</p>
                  </article>
                ))}
              </div>
            </section>
